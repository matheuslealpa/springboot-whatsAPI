package com.springbootwhatsapi.core.repository.datafilter;
...
public class RSQLParam {
    ...
    public static class AppArgumentParse extends DefaultArgumentParser {
        @SuppressWarnings("unchecked")

        @Override
        public <T> T parse(String argument, Class<T> type) throws ArgumentFormatException, IllegalArgumentException {
            if (type.equals(LocalDate.class)) return (T) LocalDate.parse(argument);
    ...
    public <T> Specification<T> getSpecification() {
        ...
        return (root, cq, cb) -> {
            ...
            predicateVisitor.getBuilderTools().setArgumentParser(new AppArgumentParse());
